...
focusSlide.prototype = {	
	...
	creatThumb: function(){
		//top导航
		if( this.outertop.children().length == 0 ){
			var outertop_html = "";
			for(var i=0; i<this.slideList.children().length; i++){
				outertop_html += "<li></li>";
			}
			this.outertop.html(outertop_html);
		}
		this.outertop.children().eq(this.currentIdx).addClass("currentd");
	...
	setCurrent: function(currentIdx){
		this.outertop.children().eq(currentIdx).addClass("currentd").siblings().removeClass("currentd");
	...
	bind: function(){
		...
		me.thumb.on("click", function(e){
			clearInterval(me.curInterval);
			me.currentIdx = jQuery(e.target).index();
